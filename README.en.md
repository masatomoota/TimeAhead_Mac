...
- Local install script: `scripts/install_app.sh`
...
## Install on this Mac and launch at login
```bash
./scripts/install_app.sh
```

- Rebuilds `build/TimeAhead.app` before copying it to `/Applications/TimeAhead.app`.
- Writes `~/Library/LaunchAgents/com.masatomoota.timeahead.plist` and reloads it with `launchctl`.
- Starts the app with `--no-prompt-on-launch` for login-time background launch.
